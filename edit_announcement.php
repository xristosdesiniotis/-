...
if (isset($_GET['id'])) {
    ...
    // Ανάκτηση των δεδομένων της συγκεκριμένης ανακοίνωσης από τη βάση δεδομένων
    $sql = "SELECT subject, date, content FROM announcements WHERE id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $id);
    $stmt->execute();
    $stmt->bind_result($subject, $date, $content);
    $stmt->fetch();
    $stmt->close();
} else {
...
<body>
    ...
    <div class="container">
        ...
        <form action="update_announcement.php" method="post">
            ...
            <input type="text" id="date" name="date" value="<?= htmlspecialchars($date) ?>" required>
